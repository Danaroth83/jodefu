...
%      - grad:     handle for the gradient operator of f(x)
...
function [x_out,cost_out]=Loris_new(varargin)

%% Parse inputs

if rem(numel(varargin),2)~=0
    fprintf(['Usage: [x,cost]=Loris(''y'',y,''Nbiter'',Nbiter,''opf'',opf,''opA'',opA,''opL'',opL,','''opg'',opg)\n']);
...
for ii=1:2:numel(varargin)
    ...
    elseif any(strcmpi(pname,{'f','opf','grad','fidelity'}))
        if isfield(pval,'grad')
            if nargin(pval.grad)==1, grad_f=pval.grad;
            elseif nargin(pval.grad)==2, grad_f=@(x) pval.grad(x,1);
            else, error('The gradient operator is not properly formatted');
            end
        else, error('The gradient operator is not properly formatted');
        end
        if isfield(pval,'direct')
...
for kk=1:Nlambda
    ...
    % Iteration 0
    b_tmp=opA(x);
    b=opAadj(grad_f(b_tmp))+c;
    if cond_stop, cost_old=norm_handle_f(b_tmp)+sum(x(:).*c(:))+norm_handle_g(opL(x),lambda(kk)); end
    ...
    if cond_stop
        cost=norm_handle_f(b_tmp)+sum(x(:).*c(:))+norm_handle_g(opL(x),lambda(kk)); 
    ...
    while iter < Nbiter && Ncor<Npos
        ...
        if cond_stop
            cost_new=norm_handle_f(b_tmp)+sum(x(:).*c(:))+norm_handle_g(opL(x),lambda(kk));
