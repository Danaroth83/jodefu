function output = swt2_Bspline(x,pow,order)
...
for k = 1:n
    ...
    lo=sqrt(fac(k))/fac(k)^(order+1)*lo;
    
    %upsample filter
    lo(2:fac_prod(k),length(lo))=0;
    lo=lo(:)';
    ...
    first = [lf,lf];
    x = wextend('2D',modeDWT,x,[floor(lf/2),floor(lf/2)]);
