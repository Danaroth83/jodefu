%% New Total Variation Operator (Variant)
%
% Description:
...
% This is a variant of the proposed approach with a 4 times, instead of 3
% times, finer grid for the gradient field. In other words, the gradient is 
% defined at pixel corners, in addition to pixel centers and pixel edges.
...
function y=TVpm_direct(x)
    ...
    y=zeros(size(x,1),size(x,2),size(x,3),8,size(x,5));    
    y(:,:,:,1,:)=x(:,:,:,1,:);
    ...
    y(1,:,:,3,:) = x(1,:,:,1,:)/2;
    y(1:end-1,1:end-1,:,4,:)=(x(1:end-1,1:end-1,:,1,:)+x(1:end-1,2:end,:,1,:))/2;
    
    y(:,:,:,5,:)=x(:,:,:,2,:);
    y(1:end-1,2:end,:,6,:)=(x(2:end,1:end-1,:,2,:)+x(1:end-1,1:end-1,:,2,:)+x(2:end,2:end,:,2,:)+x(1:end-1,2:end,:,2,:))/4; 			
    y(1:end-1,1,:,6,:)=(x(1:end-1,1,:,2,:)+x(2:end,1,:,2,:))/4;
    y(:,2:end,:,7,:) = (x(:,2:end,:,2,:)+x(:,1:end-1,:,2,:))/2; 
    y(:,1,:,7,:) = x(:,1,:,2,:)/2;
    y(1:end-1,1:end-1,:,8,:) = (x(1:end-1,1:end-1,:,2,:)+x(2:end,1:end-1,:,2,:))/2;

    y=reshape(y,[sizes(1),sizes(2),sizes(3),8,sizes(5:end)]);
...
%     y(:,:,:,4,:)=(x(:,:,:,1,:)+circshift(x(:,:,:,1,:),-1,2))/2;
%     y(:,:,:,6,:)=(x(:,:,:,2,:)+circshift(x(:,:,:,2,:),1,2)+circshift(x(:,:,:,2,:),-1,1)+circshift(circshift(x(:,:,:,2,:),-1,1),1,2))/4; 
%     y(:,:,:,7,:)=(x(:,:,:,2,:)+circshift(x(:,:,:,2,:),1,2))/2;
%     y(:,:,:,8,:)=(x(:,:,:,2,:)+circshift(x(:,:,:,2,:),-1,1))/2;
%     
%     y([1,end],:,:,:,:)=[]; y(:,[1,end],:,:,:)=[];
%     
%     y=reshape(y,[sizes(1),sizes(2),sizes(3),8,sizes(5:end)]);
