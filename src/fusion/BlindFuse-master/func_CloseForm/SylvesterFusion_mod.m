function [VX]=SylvesterFusion_mod(VXH_int,VXM,psfY,psfZ,ChInv,CmInv,E_hyper,Xd_dec,sizeIM,N_AO,prior)
...
%     VX_FFT  =   FastFusion(meanVIm,Q,Cc,InvDI,InvLbd,C2Cons,B2Sum,FBs,FBCNs,sizeMAT,sizeIM,n_dr,n_dc,psfY.ds_r);
    ...
    elseif strcmp(prior,'NLM')
        for k=1:L
            % Xd_dec(:,:,k) = NLmeansfilter(reshape(VX(k,:)',[nr nc]),5,2,invCov(k,k));
            Xd_dec(:,:,k) = NLM_personal(reshape(VX(k,:)',[nr nc]),11,1,nr*nc,invCov(k,k));
    %         Im = restoreFromSupp(reshape(VX(k,:)',[nr nc]), D,supp);
        end
    end
end