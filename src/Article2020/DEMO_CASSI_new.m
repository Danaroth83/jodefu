...
% Tests_ii=1; Tests_jj=7;
Tests_ii=2; Tests_jj=[1,4];

ra_choice=1.4;

for ii=Tests_ii   
    if ii==1, testtype='default';  lambda_v=logspace(-4,-3,5); ra=[]; end
    if ii==2, testtype='default';  lambda_v=logspace(-4,-3,5); ra=ra_choice; end
    if ii==3, testtype='msonly'; lambda_v=logspace(-3,-2,5); ra=[]; end
    if ii==4, testtype='msonly'; lambda_v=logspace(-3,-2,5); ra=ra_choice; end    
    if ii==5, testtype='nomask'; lambda_v=logspace(-4.5,-3.5,5); ra=[]; end
    ...
    if ii==8, testtype='nomask'; lambda_v=logspace(-3.5,-2.5,5); ra=[]; end
    
    if ii<=5, sim_string=0; SNR=[]; preproc='none'; end
    if ii>5,  sim_string=1; SNR=25; preproc='none'; end
