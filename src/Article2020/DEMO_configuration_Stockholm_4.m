...
for ii=Tests_ii   
    ...
    if ii==4, testtype='msonly'; lambda_v=logspace(-3.2,-2.2,5); ra=ra_choice; end    
    if ii==5, testtype='nomask'; lambda_v=logspace(-3.5,-2.5,5); ra=[]; end
    
    if ii==6, testtype='default';  lambda_v=logspace(-4,-3,5); ra=[]; end
    if ii==7, testtype='default';  lambda_v=logspace(-4,-3,5); ra=ra_choice; end
