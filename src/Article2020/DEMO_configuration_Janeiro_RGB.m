...
% im_tag='Beijing_cut256_WV3_WV3_RGB'; mask='Bayer'; alpha=[];
ratio=2;
...
for ii=Tests_ii   
    if ii==1, testtype='default';  lambda_v=logspace(-3,-2,5); ra=[]; end
    if ii==2, testtype='default';  lambda_v=logspace(-3,-2,5); ra=ra_choice; end
    if ii==3, testtype='msonly'; lambda_v=logspace(-3.5,-2.5,5); ra=[]; end
    if ii==4, testtype='msonly'; lambda_v=logspace(-3.5,-2.5,5); ra=ra_choice; end    
    if ii==5, testtype='nomask'; lambda_v=logspace(-3,-2,5); ra=[]; end
