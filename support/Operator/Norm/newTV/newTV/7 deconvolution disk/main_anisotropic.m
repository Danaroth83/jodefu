function main 	
	...
	prox_mu_gamma_g = @(t) t-t./max(abs(t)/(lambda*mu*gamma),1);
	
	x = y;
	u = zeros([size(x) 2]);
	v = opD(x);
	fprintf('0 %f\n',sum(sum((opblur(x)-y).^2))/2+lambda*sum(sum(sum(abs(v)))));
	for iter = 1:Nbiter
		...
		if mod(iter,40)==0
			...
			fprintf('%d %f\n',iter,sum(sum((opblur(x)-y).^2))/2+lambda*sum(sum(sum(abs(opD(x))))));
			figure(3);
			imshow(x);
			colormap flag
			drawnow
