function main
	...
	opD = @(z) cat(4,cat(2,diff(z,1,2),zeros(K,1,width)),cat(3,diff(z,1,3),zeros(K,height,1)));
 	...
 	prox_sigma_g_conj = @(u) bsxfun(@rdivide, u, max(sqrt(sum(u.^2,4))/lambda,1));
	...
	fprintf('0 %f\n',(sum(z(:).*p(:)) + lambda*sum(sum(sum(sqrt(sum(opD(z).^2,4))))))/2);	
	for iter = 1:nbiter		
		...
		if mod(iter,10)==0
			fprintf('%d %f %f\n',iter,(sum(znew(:).*p(:)) + ...
				lambda*sum(sum(sum(sqrt(sum(opD(znew).^2,4))))))/2,...
