%%       Discrete Total Variation based relaxed image inpainting
% 
...
verbose = true; 
video = true; 
[u,E] = solver_tv_relax(u0,lambda,A,adj_A,L_A,niter,'init',init,'gain',gain,'verbose',verbose,'video',video); 
...
solver_tv_relax(u0,lambda,A,adj_A,L_A,niter,'init',init,'gain',gain,'verbose',true,'video',true); 

% display only a particular region-of-interest
crop_roi = @(u)u(100+(1:200),100+(1:200)); 
solver_tv_relax(u0,lambda,A,adj_A,L_A,niter,'init',init,'gain',gain,'verbose',true,'video',true,'displayFcn',crop_roi); 
...
solver_tv_relax(u0,lambda,A,adj_A,L_A,niter,'init',init,'gain',gain,'verbose',true,'video',true,'displayFcn',logdft); 
...
solver_tv_relax(u0,lambda,A,adj_A,L_A,niter,'init',init,'gain',gain,'verbose',true,'video',true,'displayFcn',displayFcn); 
