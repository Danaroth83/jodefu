function [g] = sgrad(u,n)
  ...
  % + g : Shannon gradient along X-axis (horizontal) and Y-axis (vertical)
  % (concatenation on 4D dimension)
  %
  % v1.0 (12/2016): initial version.
  % v1.1 (03/2020): Modified version by Daniele Picone to adapt to color images
  
  %% Initialization
  [N,M,Nb] = size(u); % size of the image domain Omega (M = width, N=height).
  ...
  dft_u = fft2(u);
  dft_u_augm = repmat(cof,[1,1,Nb]) .* dft_u(1+mod(N+b,N),1+mod(M+a,M),:);

  %% compute (dft_gx,dft_gy) = DFT(Shannon gradient of u) 
  dft_gx = zeros(nN,nM,Nb);
  dft_gy = zeros(nN,nM,Nb);
