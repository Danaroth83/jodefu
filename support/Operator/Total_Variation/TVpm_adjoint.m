...
%
% Description:
% This function applies the adjoint of the variant version of the total 
% variation operator  described in [1], and takes the 8 obtained gradients 
% concatenated over the 4th dimension to return 2.
...
%     - Column gradient 4: y(:,n,4)= (x1(:,n+1)+x1(:,n))/2
%     - Row gradient 1:    y(:,:,5)=  x2
%     - Row gradient 2:    y(m,n,6)= (x2(m,n)+x2(m+1,n)+x2(m,n-1)+x2(m+1,n-1))/4
%     - Row gradient 3:    y(:,n,7)= (x2(:,n)-x2(:,n-1))/2
%     - Row gradient 4:    y(:,n,8)= (x2(m+1,:)-x2(m,:))/2
%
%
% Usage:
% y=TVpm_adjoint(x);
%
% Input:
% x: An Nd matrix who has the 8 gradients over the 4th dimension
%
% Output:
...
function u=TVpm_adjoint(t)
    ...
    u(1:end-1,2:end,:,1,:)=t(1:end-1,2:end,:,1,:)+(t(1:end-1,2:end,:,2,:)+t(1:end-1,1:end-1,:,2,:)+...
	                       t(2:end,2:end,:,2,:)+t(2:end,1:end-1,:,2,:))/4+...
                           (t(1:end-1,2:end,:,3,:)+t(2:end,2:end,:,3,:))/2+...
                           (t(1:end-1,1:end-1,:,4,:)+t(1:end-1,2:end,:,4,:))/2;
	u(1:end-1,1,:,1,:)=t(1:end-1,1,:,1,:)+(t(1:end-1,1,:,2,:)+t(2:end,1,:,2,:))/4+...
	                   (t(1:end-1,1,:,3,:)+t(2:end,1,:,3,:))/2+...
                       t(1:end-1,1,:,4,:)/2;
	u(2:end,1:end-1,:,2,:)=t(2:end,1:end-1,:,5,:)+(t(2:end,1:end-1,:,6,:)+t(1:end-1,1:end-1,:,6,:)+...
	                       t(2:end,2:end,:,6,:)+t(1:end-1,2:end,:,6,:))/4+...
                           (t(2:end,1:end-1,:,7,:)+t(2:end,2:end,:,7,:))/2+...
                           (t(1:end-1,1:end-1,:,8,:)+t(2:end,1:end-1,:,8,:))/2;
	u(1,1:end-1,:,2,:)=t(1,1:end-1,:,5,:)+(t(1,1:end-1,:,6,:)+t(1,2:end,:,6,:))/4+...
	                   (t(1,1:end-1,:,7,:)+t(1,2:end,:,7,:))/2+...
                       t(1,1:end-1,:,8,:)/2;
...
%       (x(:,:,:,4,:)+circshift(x(:,:,:,4,:),1,2))/2 ;
%   
%     y(:,:,:,2,:)=x(:,:,:,5,:)+...
%                 (x(:,:,:,6,:)+circshift(x(:,:,:,6,:),1,1)+circshift(circshift(x(:,:,:,6,:),1,1),-1,2)+circshift(x(:,:,:,6,:),-1,2))/4+...
%                 (x(:,:,:,7,:)+circshift(x(:,:,:,7,:),-1,2))/2+...
%                 (x(:,:,:,8,:)+circshift(x(:,:,:,8,:),1,1))/2;
%             
%     y([1,end],:,:,:,:)=[]; y(:,[1,end],:,:,:)=[];
%             
