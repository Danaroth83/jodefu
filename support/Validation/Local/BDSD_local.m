...
%       each MS band
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function I_Fus_BDSD = BDSD_local(I_MS,I_PAN,ratio,sensor,S,offset,ng_check,interp)
...
%             pan_LP_d = pan_LP(3:ratio:end,3:ratio:end);
        pan_LP_d = imresize(pan_LP,1/ratio,'nearest');
            ...
            ms_LP_LP = MTF(I_MS,sensor,[],ratio);
            if offset
...
% %                     BlockMS=MS(starty:endy,startx:endx,:);
% %                     BlockPAN=PAN(starty:endy,startx:endx);
%                     startxLP=ceil(startx/ratio);
%                     startyLP=ceil(starty/ratio);
%                     endxLP=ceil(endx/ratio);
%                     endyLP=ceil(endy/ratio);
%                     BlockMS_LP_d=ms_LP_d(startyLP:endyLP,startxLP:endxLP,:);
%                     BlockPAN_LP_d=pan_LP_d(startyLP:endyLP,startxLP:endxLP,:);
%                     BlockMS_LP=ms_orig(startyLP:endyLP,startxLP:endxLP,:);
                   %%%% Alternatively (It works better)
                    startx = max(x - floor(BlockSize/2), 1);
                    starty = max(y - floor(BlockSize/2), 1);
                    endy = min(y + floor(BlockSize/2), size(I_MS,1));
                    endx = min(x + floor(BlockSize/2), size(I_MS,2));
                    %No decimation
                    BlockMS_LP=I_MS(starty:endy,startx:endx,:);
                    BlockPAN_LP_d=I_PAN(starty:endy,startx:endx);
                    BlockMS_LP_d=ms_LP_LP(starty:endy,startx:endx,:);
%                     BlockMS_LP_d=ms_LP_d(startyLP:endyLP,startxLP:endxLP,:);
%                     BlockPAN_LP_d=pan_LP_d(startyLP:endyLP,startxLP:endxLP,:);
%                     BlockMS_LP=ms_orig(startyLP:endyLP,startxLP:endxLP,:);
                    if offset
