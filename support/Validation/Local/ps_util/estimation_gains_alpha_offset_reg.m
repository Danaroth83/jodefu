function [g] = estimation_gains_alpha_offset_reg(imageLR,imageHR,GT,type_estimation,lambda)
...
%       keyboard
%     g = ([ILRc,IHc,ones(numel(IHc),1)]+lambda*eye(size([ILRc,IHc,ones(numel(IHc),1)])))\Diffc;
        eye_size=size([ILRc,IHc,ones(numel(IHc),1)]'*[ILRc,IHc,ones(numel(IHc),1)]);
%     g= ([ILRc,IHc,ones(numel(IHc),1)]'*[ILRc,IHc,ones(numel(IHc),1)]+lambda*eye(eye_size))^-1*[ILRc,IHc,ones(numel(IHc),1)]'*Diffc;
    g= ([ILRc,IHc,ones(numel(IHc),1)]'*[ILRc,IHc,ones(numel(IHc),1)]+lambda*eye(eye_size))\[ILRc,IHc,ones(numel(IHc),1)]'*Diffc;
