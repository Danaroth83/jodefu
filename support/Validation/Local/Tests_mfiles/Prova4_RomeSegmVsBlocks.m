...
MatrixImage(:,:,:,4) = GS_local(MS, PAN, I_LP_input,Sbpt_MS(:,:,4),equaliz,interp);
MatrixImage(:,:,:,5) = GS_local(MS, PAN, I_LP_input,Sbpt_MS(:,:,5),equaliz,interp);
MatrixImage(:,:,:,6) = GS_local(MS, PAN, I_LP_input,Sbpt_MS(:,:,7),equaliz,interp);
MatrixImage(:,:,:,7) = GS_local(MS, PAN, I_LP_input,100,equaliz,interp);
MatrixImage(:,:,:,8) = GS_local(MS, PAN, I_LP_input,60,equaliz,interp);
MatrixImage(:,:,:,9) = GS_local(MS, PAN, I_LP_input,30,equaliz,interp);

titleImages = {'HRMS','MS',['BPT Segm ',num2str(nregs(1))],['BPT Segm ',num2str(nregs(5))],...
