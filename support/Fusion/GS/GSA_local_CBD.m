...
function [I_Fus_GSA,alpha,g,Correlation_map] = GSA_local_CBD(I_MS,I_PAN,~,ratio,block_win,block_shift,flag_downsample,threshold)
...
for ii = 1 : stepx
    ...
    for jj = 1 : stepy
        ...
        if Correlation_map(ii,jj)>threshold
            for zz = 1 : Nb
                h = imageLR0_min(:,:,zz);
                c = cov(I0block(:),h(:));
            ...
            g(ii,jj,2:Nb+1) = zeros(1,1,Nb);
        end
%         if Correlation_map(ii,jj)>threshold
%             g(ii,jj,2:Nb+1) = repmat(std2(imageLR0_ext(iA,iB,:))/std2(imageHR_ext(iA,iB)),[1,1,Nb]);
%         else
%             g(ii,jj,2:Nb+1) = zeros(1,1,Nb);
%         end
