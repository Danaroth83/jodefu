...
function [I_Fus_GSA,alpha,g,Correlation_map] = GSA_local_CBD_corr(I_MS,I_PAN,~,ratio,block_win,block_shift,flag_downsample)
...
delta=zeros(size(I_MS,1),size(I_MS,2));
V=zeros(size(I_MS,1),size(I_MS,2),size(I_MS,3)+1);
for ii = 1 : stepx
...
%             h = imageLR0_min(:,:,zz);
%             c = cov(I0block(:),h(:));
%             g(ii,jj,zz+1) = Correlation_map(ii,jj)*c(1,2)/var(I0block(:));
%         end
        g(ii,jj,2:Nb+1) = repmat(std2(imageLR0_ext(iA,iB,:))/std2(imageHR_ext(iA,iB)),[1,1,Nb])*Correlation_map(ii,jj);

        delta(iA1,iB1)=imageHRblock-I0minblock;
        V(iA1,iB1,:)=cat(3,I0minblock,imageLR0_min(iA2,iB2,:));
    end
end
Vhat=V+repmat(delta,[1,1,size(I_MS,3)+1]).*g/median(Correlation_map(:));
I_Fus_GSA=Vhat(:,:,2:end);
