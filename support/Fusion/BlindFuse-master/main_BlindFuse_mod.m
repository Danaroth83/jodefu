function [I_out,BluKer,time]=main_BlindFuse_mod(PAN,MS,ratio,intersection,start_pos,prior,subMeth)

if nargin<=2, ratio=size(PAN,1)/size(MS,1); end
if nargin<=3, intersection=cell(1,size(PAN,3)); intersection(:)={1:size(MS,3)}; end
if nargin<=4, start_pos=[floor(ratio/2)+1,floor(ratio/2)+1]; end
if nargin<=5, prior='Gaussian'; end  % other prior: 'Sparse', 'ML'
if nargin<=6, subMeth='PCA'; end % other component analysis: 'VCA', 'HySime'
shift=floor(mean(start_pos))-1;
warning('off','verbose');
addpath([pwd,'/func_CloseForm']);
if strcmpi(prior,'Sparse'), addpath([pwd,'/func_Sparse']); end
if strcmpi(prior,'NLM'), addpath([pwd,'/func_NLM']); end

% original: const=max(PAN(:)); MS=MS/const; PAN=PAN/const;
...
% MS=double(imread('DE2_MS4_L1+C_000000_20150530T183126_20150530T183129_DE2_5119_FC14.tif'));MS=MS/const;MS=rot90(MS,3);%MS=permute(MS,[2 1 3]);
% MSband_show=[2 3 4];
...
lambda_R = 1e1;
lambda_B = 1e1;
p=min(20,size(MS,3)); %p=4; %p=20; for Qunming' data
...
scale=1; [E_hyper, ~, ~, ~]=idHSsub(MS,subMeth,scale,p);
...
tic
I_out=BayesianFusion(MS,PAN,BluKer,ratio,R_est,E_hyper,prior,start_pos);
time=toc;
