function [X_BayesFusion]= BayesianFusion(XH,XM,KerBlu,ratio,psfZ,E_hyper, prior,start_pos)
...
XH_int(1:psfY.ds_r:end,1:psfY.ds_r:end,:)=XH;
VXH_int=reshape(XH_int,[nr*nc N_HS])'; 
%% The spline interpolated image as the prior mean of target image
% Xd_dec = ima_interp_spline(var_dim(XH,P_dec),psfY.ds_r); % give the prior in subspace
Xd_dec = var_dim(ima_interp_spline(XH,psfY.ds_r),P_dec);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% load('I_Hysure.mat');
% Xd_dec=I_HySure;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
VX=SylvesterFusion_mod(VXH_int,VXM,psfY,psfZ,ChInv,CmInv,E_hyper,Xd_dec,sizeIM,N_AO,prior);
% [HSFusion.(FusMeth),Costime,diff_X,RMSE_sub,RMSE_org,invCov_U,VX_dec_set]=AlterOpti(XHd_int,XH,XM,psfY,...
%     psfZ_unk,sigma2y_real,sigma2z_real,P_dec,P_inc,MatScale,FusMeth,X_real,ADMM,Para_Prior);
