function [X_BayesFusion]= BayesianFusion_mod2(XH,XM,overlap,KerBlu,ratio,prior,start_pos)
...
scale  =1;
[E_hyper, P_dec, ~, L]=idHSsub_mod(XH,subMeth,scale);
sizeIM=[nr,nc,L];
%% Spectral mixture: spectral response R
psfZ=zeros(N_MS,N_HS);
for ii=1:N_MS
    psfZ(ii,overlap{ii})=ones(size(overlap{ii}))/length(overlap{ii});
end
...
if strcmp(prior,'Sparse')
    ...
    if learn_dic==1
        [time_LD,D_s]=Dic_Para_mod(XM_shift,E_hyper,learn_dic,train_support,0,inf);
...
if exist('Dic_Learn.m', 'file') == 2
    delete('Dic_Learn.m');
end
if exist('test.m', 'file') == 2
    delete('test,m');
end
