...
function [x_out,cost_out]=Chambolle_new(varargin)
...
for kk=1:Nlambda
    ...
    while iter < Nbiter && Ncor < Npos
        if rho==1
            ...
            u=prox_gamma_g_conj(u+sigmatau*opL(xtilde-l),sigma,lambda(kk));
            l=l+opLadj(u)-b;
            v=prox_gamma_f_conj(v+etatau*opA(xtilde-2*l),eta);
            xtilde=xtilde-l;
            l=opAadj(v)+b+c;
        else
            % Over-relaxed version
            u1=prox_gamma_g_conj(u+sigmatau*opL(xtilde-l),sigma,lambda(kk));
            l1=l+opLadj(u1)-b;
            v1=prox_gamma_f_conj(v+etatau*opA(xtilde-2*l1),eta);
            xtilde=xtilde-rho*l1;
            v=v+rho*(v1-v);
            u=u+rho*(u1-u);
            b=b+rho*(l1-l);
            l=opAadj(v)+b+c;
